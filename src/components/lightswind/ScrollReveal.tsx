...
export function ScrollReveal({
  ...
  return (
    ...
    >
      <motion.p
        className={cn(
          "leading-relaxed font-semibold !text-text-primary",
          sizeClasses[size],
          alignClasses[align],
        ...
        {splitText.map(
          (
            ...
            item.isSpace ? (
              ...
              <span key={`space-${item.originalIndex} !text-text-primary`}>
                {item.value}
              </span>
            ) : (
              // Render words as motion.span for animation
              <motion.span
                key={`word-${item.originalIndex} !text-text-primary`} // Use originalIndex for stable keys
